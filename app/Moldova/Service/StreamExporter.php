...
class StreamExporter
{
    ...
    {
        $data     = [];
        $response = new StreamedResponse(function () use ($data, $value, $column) {
            set_time_limit(0);
    ...
    public function fetchAgencies()
    {
        ...
        $response = new StreamedResponse(function () use ($data) {
            ...
            $this->ocdsRelease->distinct('buyer.name')->chunk(400, function ($buyers) use ($data, $handle) {
    ...
    public function fetchTenders()
    {
        ...
        $response = new StreamedResponse(function () use ($data) {
            ...
            $this->ocdsRelease->chunk(400, function ($tenders) use ($data, $handle) {
    ...
    public function fetchTenderGoods($tenderId)
    {
        ...
        $response = new StreamedResponse(function () use ($data, $tenderId) {
            set_time_limit(0);
            $handle = fopen('php://output', 'w');
            fputcsv($handle, [
                'name',
